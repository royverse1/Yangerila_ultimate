...
const FooterReveal = React.memo(function FooterReveal({ step }) {
  ...
  useGSAP(() => {
    function startTickers() {
      ...
      tl1.current?.play(); tl2.current?.play(); tl3.current?.play();
    }
    ...
    <div className="w-full flex whitespace-nowrap overflow-visible relative py-1 md:py-2">
      <div ref={ref} className="flex gap-3 md:gap-4 items-center px-4 w-max text-left">
        {[...testimonials, ...testimonials, ...testimonials].map((t, idx) => (
          <div key={idx} onClick={handleCardClick} className="testimonial-card relative bg-gradient-to-br from-[#0c1821] to-[#040a0f] p-5 lg:p-6 rounded-[1.5rem] md:rounded-[2rem] border border-white/10 w-[65vw] md:w-[320px] lg:w-[340px] whitespace-normal flex-shrink-0 transition-all duration-500 overflow-visible group cursor-pointer">
  ...
  return (
    <div ref={containerRef} className="w-full flex flex-col shrink-0 pointer-events-auto">
      ...
      {/* SECTION 10: Fun Fact */}
      <div ref={funFactRef} className="w-full h-screen flex flex-col items-center justify-center text-center bg-[linear-gradient(to_bottom,#0d2a1c,var(--color-pitch-black))] relative px-6 md:px-12 border-t border-emerald-900/50 shrink-0">
        <div className="max-w-5xl mx-auto flex flex-col items-center justify-center w-full">
          <h2 className="fun-fact-el text-neon-mint tracking-[0.3em] font-bold text-sm uppercase mb-12 drop-shadow-[0_0_15px_rgba(46,211,162,0.8)] invisible">Fun Fact</h2>
          <p className="fun-fact-el text-4xl md:text-6xl text-white font-light text-serif-italic mb-12 leading-relaxed drop-shadow-xl invisible">
            "Fender published a study stating that about 90% of guitar students quit playing within their first year."
          </p>
          <div className="fun-fact-el w-full h-[1px] bg-white/10 my-12 relative overflow-hidden max-w-5xl invisible">
            <div className="absolute inset-y-0 left-0 bg-neon-mint w-1/3 blur-md shadow-[0_0_20px_#2ed3a2]"></div>
          </div>
          <h3 className="fun-fact-el text-4xl md:text-6xl text-white font-black uppercase tracking-tighter drop-shadow-2xl invisible">
          ...
          <p className="fun-fact-el text-xl text-neutral-300 font-light mt-8 invisible">This opposite statistic fills us with both happiness and confidence in our teaching methods.</p>
        </div>
      </div>

      {/* SECTION 11: Voices */}
      <div ref={voicesRef} className="w-full h-screen overflow-hidden flex flex-col justify-between bg-[linear-gradient(to_bottom,#08273d,var(--color-pitch-black))] border-t border-blue-900/50 relative py-10 lg:py-16 px-0 shrink-0">
        <div className="voices-header shrink-0 flex flex-col px-6 md:px-12 xl:px-24 invisible">
          ...
          <p className="text-neutral-300 mt-3 md:mt-4 max-w-xl text-xs md:text-sm lg:text-base opacity-80">“Out of hundreds of testimonials over the years, we’ve handpicked a few.”</p>
        </div>

        <div
          className="voices-rows flex-1 w-full flex flex-col justify-center gap-0 md:gap-1 overflow-visible relative pointer-events-auto mt-4 md:mt-8 invisible"
