...
const HeroReveal = React.memo(function HeroReveal({ step, onComplete, isReversing }) {
  ...
  useGSAP(() => {
    // 1. EXIT SCENARIO (Forward to Step 3)
    if (step > 2) {
    ...
    // 2. ENTER SCENARIO (Reverse from Step 3 back to Step 2 - About)
    if (isReversing && step === 2) {
      ...
      // Snap Hero elements off, About elements on
    ...
    // 3. NORMAL STEP LOGIC (Forward movements)

    // Step 0: Initial Static Hero
    if (step === 0) {
      if (isReversing) {
    ...
    // Step 1: ZOOM 'Y', Title, AND Paragraph load together
    if (step === 1) {
      if (isReversing) {
    ...
    // Step 2: About Yangerila Reveal (Newspaper style typewriter effect)
  ...
  return (
    <section
      ref={containerRef}
      // CRITICAL: z-50 ensures Hero is always on top of Legacy during its upward slide exit
